...
(* ::Input:: *)
lmax=1
wD[l_,m1_,m2_]:=wD[l,m1,m2]=Block[{\[Alpha]=\[Pi]/2,\[Beta]=\[CapitalTheta],\[Gamma]=\[CapitalPhi]},ComplexExpand[WignerD[{l,m1,m2},\[Alpha],\[Beta],\[Gamma]]]]

rotateYlmRules=Flatten[Table[ysph[l,m]->Sum[wD[l,\[Mu],m]*ysph[l,\[Mu]],{\[Mu],-l,l}],{l,0,lmax},{m,-l,l}]]
ysph2real[l_,m_]:=ysh2real[l,m]=(-1)^m*Which[
...
yreal2ysph[l_,m_]:=yreal2ysph[l,m]=(-1)^m*Which[
...
rotatedYreal[l_,m_]:=rotatedYreal[l,m]=((yreal[l,m]/.yreal2ysphRules)/.rotateYlmRules)/.ysph2yrealRules
...
(* ::Input:: *)
multipoleIntegrand[l1_,m1_,lM_,mM_,l2_,m2_]:=multipoleIntegrand[l1,m1,lM,mM,l2,m2]=TensorExpand[
...
zeroMultipoleRules=Flatten[Table[multipoleRule[l1,m1,lM,mM,l2,m2],{l1,0,lmax},{l2,0,lmax},{lM,1,1},{m1,-l1,l1},{m2,-l2,l2},{mM,-1,1}]]
...
(* ::Input:: *)
SlaterKosterMultipole[l1_,m1_,lM_,mM_,l2_,m2_]:=SlaterKosterMultipole[l1,m1,lM,mM,l2,m2]=FullSimplify[((TensorExpand[( 
( 
 TrigExpand[Simplify[ComplexExpand[Echo[multipoleIntegrand[l1,m1,lM,mM,l2,m2],"multipole integrand:"]],realAssumptions]]//TransformedField["Spherical"->"Cartesian",#,{R,\[CapitalTheta],\[CapitalPhi]}->{x,y,z}]&)
...
Off[ParallelMap::subpar]  (* turn off warnings *)
DistributeDefinitions[SlaterKosterMultipole]
Print["Slater-Koster Transformations"]
SKtransfListxyz=Parallelize[Outer[Echo[{
...
SlaterKosterMultipole[Part[#1,2],Part[#1,3],Part[#2,2],Part[#2,3],Part[#3,2],Part[#3,3]]
	},"Slako:"]&,orbitalslm,{{py,1,-1},{pz,1,0},{px,1,1}},orbitalslm,1,1,1], Method->"FinestGrained"]//Flatten[#,2]&
...
fh=OpenWrite["transformations_dipole_lmax"<>ToString[lmax]<>".py"]
WriteString[fh,"# This file has been generated automatically\n"]
WriteString[fh,"lmax = "<>ToString[lmax]<>"\n"]
...
(* FORTRAN CODE *)
Print["Fortran Code"]
skFile="slako_transformations_dipole_lmax"<>ToString[lmax]<>".f90"
Close[skFile]
